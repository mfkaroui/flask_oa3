...
class TestServerVariable:
    ...
    def test_variable_specification_extensions(
        self, server_variable_specification_extensions_fixture
    ):
        ...
        assert (
            server_variable_specification_extensions_fixture.oa3_schema
            == expected_schema
        )
    ...
    def test_server_variable_no_description_oa3_schema(
        self, server_variable_with_enum_no_description_fixture
    ):
        expected_schema: dict = {"enum": ["test1", "test2"], "default": "test1"}
        assert (
            server_variable_with_enum_no_description_fixture.oa3_schema
            == expected_schema
        )

    def test_server_variable_just_default_oa3_schema(
        self, server_variable_just_default_fixture
    ):
        expected_schema: dict = {"default": "test1"}
        assert server_variable_just_default_fixture.oa3_schema == expected_schema

    @pytest.mark.parametrize(
        ("default_value",),
        [("test3",), ("test4",), ("test5",), ("test",), ("test_1",), ("test2_",)],
    )
    ...
    @pytest.mark.parametrize(
        ("default_value",), [("test1",), ("test2",), ("test3",), ("test4",), ("test5",)]
    )
