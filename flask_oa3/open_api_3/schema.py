...
class Schema(Component):
    ...
    discriminator: Annotated[
        ...
        Field(
            default=None,
            description="Adds support for polymorphism. The discriminator is an object name that is used to differentiate between other schemas which may satisfy the payload description. See Composition and Inheritance for more details.",
        ),
    ]
    xml: Annotated[
        Optional[XML],
    ...
    def component_name(self) -> str:
        if isinstance(self.schema_model, Reference[Schema]):
        ...
        elif isinstance(self.schema_model, list) or isinstance(
            self.schema_model, tuple
        ):
            components = []
            ...
            is_one_of = (
                isinstance(self.schema_model, tuple) or self.discriminator is not None
            )
    ...
    def validate_schema_model(
        ...
        if isinstance(value, list) or isinstance(value, tuple):
            for model in value:
                if (
                    not isinstance(model, Reference[Schema])
                    and not issubclass(model, BaseModel)
                    and not issubclass(model, RootModel)
                ):
                    raise TypeError(
                        "schema_model in list must be a pydantic model or a reference to a schema"
                    )
        elif (
            not isinstance(value, Reference[Schema])
            and not issubclass(value, BaseModel)
            and not issubclass(value, RootModel)
        ):
            raise TypeError(
                "schema_model must be a pydantic model or a reference to a schema"
            )
        return value
    ...
    def validate_schema(cls, self: Schema) -> Schema:
        if self.discriminator is not None:
            if isinstance(self.schema_model, list) or isinstance(
                self.schema_model, tuple
            ):
                if not self.has_property(self.discriminator.property_name):
                    raise KeyError(
                        "schema_model must have a property that matches the discriminator property name"
                    )
            else:
                raise TypeError(
                    "schema_model must be a list or tuple when a discriminator is defined"
                )
        return self
    ...
    def schema_model_json_schema(self) -> Tuple[dict, dict]:
        ...
        if isinstance(self.schema_model, Reference[Schema]):
            ...
            for model in self.schema_model:
                if isinstance(model, Reference[Schema]):
                    ...
                    json_schema = model.model_json_schema(
                        by_alias=True, ref_template=self.component_path() + "/{model}"
                    )
            ...
            json_schema = self.schema_model.model_json_schema(
                by_alias=True, ref_template=self.component_path() + "/{model}"
            )
            json_schema_defs = json_schema.pop("$defs", None)
            if json_schema_defs is not None:
...
class Discriminator(BaseModel):
    ...
    mapping: Annotated[
        Optional[Dict[str, Union[str, Reference[Schema]]]],
        Field(
            default=None,
