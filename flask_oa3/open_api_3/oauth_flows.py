...
class OAuthFlows(BaseModel):
    implicit: Annotated[
        Optional[OAuthFlow],
        Field(default=None, description="Configuration for the OAuth Implicit flow"),
    ]
    password: Annotated[
        Optional[OAuthFlow],
        Field(
    ...
    def validate_flow_requirements(cls, self: OAuthFlows) -> OAuthFlows:
        if self.implicit is not None:
            if self.implicit.authorization_url is None:
                raise ValueError(
                    "An authorization url is required for an OAuth Implicit flow"
                )
        ...
        if self.password is not None:
            if self.password.token_url is None:
                raise ValueError(
                    "A token url is required for an OAuth Resource Owner Password flow"
                )
            if self.password.scopes is None:
                raise ValueError(
                    "Scopes are required for an OAuth Resource Owner Password flow"
                )
        if self.client_credentials is not None:
            if self.client_credentials.token_url is None:
                raise ValueError(
                    "A token url is required for an OAuth Client Credentials flow"
                )
            if self.client_credentials.scopes is None:
                raise ValueError(
                    "Scopes are required for an OAuth Client Credentials flow"
                )
        if self.authorization_code is not None:
            if self.authorization_code.authorization_url is None:
                raise ValueError(
                    "An authorization url is required for an OAuth Authorization Code flow"
                )
            if self.authorization_code.token_url is None:
                raise ValueError(
                    "A token url is required for an OAuth Authorization Code flow"
                )
            if self.authorization_code.scopes is None:
                raise ValueError(
                    "Scopes are required for an OAuth Authorization Code flow"
                )
        return self
