...
class ServerVariable(BaseModel):
    ...
    def validate_default_enum(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        if ("enum" in values and values["enum"] is not None) and (values["default"] not in values["enum"]):
            raise ValueError(f"Default value '{values['default']}' is not in enum: {values['enum']}")
        return values
